...
/**
 * Implementation of BrokerService for OANDA
 */
export class OandaService implements BrokerService {
  ...
  async connect(): Promise<void> {
    ...
    if (!this.apiToken) {
      throw new Error('OANDA API token is required');
    }
  ...
  async getBalance(): Promise<AccountBalance> {
    ...
    const accountDetails = await this.request(`/v3/accounts/${this.accountId}`);
  ...
  async getPositions(): Promise<BrokerPosition[]> {
    ...
    const positionsData = await this.request(`/v3/accounts/${this.accountId}/positions`);
  ...
  async getOrderHistory(): Promise<OrderHistory[]> {
    ...
    const orderData = await this.request(`/v3/accounts/${this.accountId}/orders`);
