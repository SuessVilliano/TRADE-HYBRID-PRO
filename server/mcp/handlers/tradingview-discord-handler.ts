...
export async function handleTradingViewDiscordWebhook(req: Request, res: Response, mcp: MCPServer): Promise<void> {
  try {
    ...
    // Get signal service to add signal to our system
    const signalService = mcp.signalService;
    if (signalService) {
      await signalService.addSignal(signal);
      console.log('Added signal to signal service');
    }
...
export async function handleTradeExecutionWebhook(req: Request, res: Response, mcp: MCPServer): Promise<void> {
  try {
    ...
    // Get user profile service to add trade to user's history
    const userProfileService = mcp.userProfileService;
    if (userProfileService && payload.userId) {
      try {
        await userProfileService.addUserTrade(payload.userId, {
          ...payload,
          id: payload.id || `trade_${Date.now()}_${Math.floor(Math.random() * 1000)}`,
          entryDate: payload.entryDate || new Date().toISOString(),
          status: 'open',
          profitLoss: 0
        });
        console.log(`Added trade to user ${payload.userId}'s history`);
      } catch (err) {
        console.error('Error adding trade to user profile:', err);
      }
    }
...
export async function handleTradeClosedWebhook(req: Request, res: Response, mcp: MCPServer): Promise<void> {
  try {
    ...
    // Get user profile service to update trade in user's history
    const userProfileService = mcp.userProfileService;
    if (userProfileService && payload.userId) {
      try {
        await userProfileService.updateUserTrade(payload.userId, payload.id, {
          ...payload,
          exitDate: payload.exitDate || new Date().toISOString(),
          status: 'closed'
        });
        console.log(`Updated trade ${payload.id} in user ${payload.userId}'s history`);
      } catch (err) {
        console.error('Error updating trade in user profile:', err);
      }
    }
