...
// Use mock service based on config or fallback to checking API keys
...
export class BrokerFactory {
  ...
  static createBrokerService(brokerType: 'alpaca' | 'alpaca-broker' | 'mock'): BrokerService {
    ...
    if (USE_MOCK_SERVICE || !config.ALPACA_API_KEY || !config.ALPACA_API_SECRET) {
      console.log('Using mock broker service (configured by flag or missing API keys)');
    ...
    try {
      switch (brokerType) {
        case 'alpaca':
          // If we have API keys, use them
          if (config.ALPACA_API_KEY && config.ALPACA_API_SECRET) {
            console.log('Using real Alpaca service with provided API keys');
        ...
        case 'alpaca-broker':
          // Fallback to mock service for now
          return new MockBrokerService();
