...
export default function ChatRoom({ 
  ...
  // Filter messages for the current room if in a room
  const filteredMessages = currentRoomId 
    ? chatMessages.filter(msg => 
        (msg.type === "trading" && msg.target === currentRoomId) ||
        (msg.type === "global" && !currentRoomId))
  ...
  const handleSendMessage = (e: React.FormEvent) => {
    ...
    // Send the message based on the context (global or trading room)
    sendChatMessage(
      messageToSend, 
      currentRoomId ? "trading" : "global", 
  ...
  const roomName = currentRoomId 
    ? tradingRooms.find(room => room.id === currentRoomId)?.name || "Trading Room"
    : "Global Chat";
  ...
  return (
    <div 
      className={cn(
        ...
        minimized ? "w-60 h-10" : "w-96 h-[500px]",
        className
      )}
    >
      {/* Chat header */}
        ...
        <div className="flex items-center gap-2">
          ...
          {currentRoomId && (
            <span className="text-xs text-gray-400">
              ({tradingRooms.find(room => room.id === currentRoomId)?.members || 0} online)
            </span>
          )}
        </div>
      ...
      {!minimized && (
        <>
          ...
          <div 
            ref={chatMessagesRef}
            className="h-[calc(100%-132px)] px-4 py-3 overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent space-y-4"
          >
              ...
              filteredMessages.map((msg) => (
                ...
                >
                  <div className={cn(
                    ...
                    msg.sender === "System" ? "bg-gray-700 text-left w-full" : 
