...
export function TradingViewCryptoHeatmap({
  ...
  const uniqueId = useRef(`tv_crypto_heatmap_${Math.floor(Math.random() * 1000000)}`);
  ...
  useEffect(() => {
    ...
    try {
      ...
      script.onerror = () => {
        setError('Failed to load crypto heatmap. Please try again later.');
        setLoading(false);
      };
      ...
      const loadTimeout = setTimeout(() => {
        if (loading) {
          setError('The crypto heatmap is taking longer than expected to load. Please try refreshing.');
        }
      }, 8000);
      ...
      setError('An error occurred while loading the crypto heatmap.');
      setLoading(false);
      console.error('Error loading crypto heatmap:', err);
    }
  }, [height, width, colorTheme, hasTopBar, isZoomEnabled]);
  ...
  const handleReload = () => {
    if (containerRef.current) {
      ...
      setTimeout(() => {
        ...
        script.onerror = () => {
          setError('Failed to reload crypto heatmap.');
          setLoading(false);
        };
  ...
  return (
    <div className={`w-full h-full bg-slate-800 rounded-md flex flex-col ${className}`}>
      <div className="p-2 border-b border-slate-700 flex justify-between items-center">
        <div className="font-medium text-sm">Crypto Market Heatmap</div>
        ...
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center bg-slate-800 bg-opacity-75 z-10">
            <div className="text-center">
              ...
              <p className="text-sm text-slate-300">Loading crypto heatmap...</p>
            </div>
          </div>
        )}
