...
interface TradingViewCryptoHeatmapWidgetProps {
  dataSource?: string;
  blockSize?: string;
  blockColor?: string;
...
function TradingViewCryptoHeatmapWidget({
  ...
  const uniqueId = useRef<string>(`tradingview_crypto_${Math.random().toString(36).substring(2, 10)}`);
  ...
  }, [dataSource, blockSize, blockColor, colorTheme, hasTopBar, isZoomEnabled, width, height]);
