...
export function TradingViewStockHeatmap({
  ...
  const uniqueId = useRef(`tv_stock_heatmap_${Math.floor(Math.random() * 1000000)}`);
  ...
  useEffect(() => {
    ...
    try {
      ...
      script.onerror = () => {
        setError('Failed to load stock heatmap. Please try again later.');
        setLoading(false);
      };
      ...
      const loadTimeout = setTimeout(() => {
        if (loading) {
          setError('The stock heatmap is taking longer than expected to load. Please try refreshing.');
        }
      }, 8000);
      ...
      setError('An error occurred while loading the stock heatmap.');
      setLoading(false);
      console.error('Error loading stock heatmap:', err);
    }
  }, [height, width, colorTheme, dataSource, grouping, hasTopBar, isZoomEnabled]);
  ...
  const handleReload = () => {
    if (containerRef.current) {
      ...
      setTimeout(() => {
        ...
        script.onerror = () => {
          setError('Failed to reload stock heatmap.');
          setLoading(false);
        };
  ...
  return (
    <div className={`w-full h-full bg-slate-800 rounded-md flex flex-col ${className}`}>
      <div className="p-2 border-b border-slate-700 flex justify-between items-center">
        <div className="font-medium text-sm">Stock Market Heatmap</div>
        ...
        {loading && (
          <div className="absolute inset-0 flex items-center justify-center bg-slate-800 bg-opacity-75 z-10">
            <div className="text-center">
              ...
              <p className="text-sm text-slate-300">Loading stock heatmap...</p>
            </div>
          </div>
        )}
