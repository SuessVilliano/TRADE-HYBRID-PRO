...
interface TradeRunnerGameProps {
  className?: string;
  embedded?: boolean;
}

export function TradeRunnerGame({ className = '', embedded = false }: TradeRunnerGameProps) {
  ...
  const gameLoop = () => {
    try {
      const canvas = canvasRef.current;
      if (!canvas) {
        console.error("Canvas reference is null in gameLoop");
        return;
      }
      
      const ctx = canvas.getContext('2d');
      if (!ctx) {
        console.error("Could not get 2D context from canvas in gameLoop");
        return;
      }
      ...
      if (gameActive) {
        ...
        console.log("Game not active, not continuing game loop");
      }
    } catch (error) {
  ...
  useEffect(() => {
    const resizeCanvas = () => {
      ...
      if (canvas) {
        ...
        if (container) {
          ...
          console.log(`Canvas resized to ${canvas.width}x${canvas.height} (no parent container)`);
        }
      }
    };
    ...
    // Give time for the component to render before setting canvas size
  ...
  return (
    <Card className={`${className} overflow-hidden h-full flex flex-col`}>
      <CardHeader className={`${embedded ? 'p-2' : 'p-4'} flex-shrink-0 bg-slate-900 border-b border-slate-800`}>
        <div className="flex justify-between items-center">
          <CardTitle className={`${embedded ? 'text-base' : 'text-lg'}`}>Trade Runner</CardTitle>
        ...
        {showControls && (
          <div className="mt-2 p-2 bg-slate-800 rounded-md text-xs">
            <h3 className="font-bold mb-1">Controls</h3>
            <div className="grid grid-cols-2 gap-1">
              <div><span className="font-bold">Move Left:</span> Left Arrow / A</div>
              <div><span className="font-bold">Move Right:</span> Right Arrow / D</div>
              <div><span className="font-bold">Jump:</span> Space</div>
              <div><span className="font-bold">Goal:</span> Collect coins</div>
            </div>
          </div>
        )}
      </CardHeader>
      
      <CardContent className="p-0 relative flex-grow bg-black">
        <canvas 
          ref={canvasRef} 
          className="w-full h-full block bg-gradient-to-b from-blue-950 to-black" 
        />
        
        {showWelcome && (
          <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black/90 p-4 border-2 border-fuchsia-500 rounded-md text-center w-4/5 max-w-[300px]">
